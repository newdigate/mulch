...
| **Audio File** | play an audio file — mp3, wav, flac, m4a, ogg, … (any FFmpeg format), decoded to 48 kHz stereo → `left`/`right` mono outputs; signed `rate` (negative = reverse), variable speed, loop. `sync` + `length` bar-lock the clip to the transport, time-warping it to span exactly `length` bars (`rate`/`loop` ignored while synced). `auto play` instead hands playback to the transport, overriding `play`: it sounds while the transport plays, holds on pause, and rewinds on stop so the next play starts from the beginning |
